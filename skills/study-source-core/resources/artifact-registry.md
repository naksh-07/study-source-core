# Artifact Registry (rtifact-registry.json)
...
## 2. Location
The registry is located at esources/artifact-registry.json and is parsed dynamically by scripts/artifact_registry.js.

## 3. Schema Structure
The registry defines the execution capabilities of all 12 study artifacts. The schema enforces:
- 	ask_id: Unique string identifier for the execution task
- 	ask_name: Human-readable task name
...
- alidator: The physical JavaScript validator script to verify the output
- rtifactKey: The identity key matching subject-policy-runtime definitions
-  alidator: The physical JavaScript validator script to verify the output
-  rtifactKey: The identity key matching subject-policy-runtime definitions
- dependencies: An array of 	ask_id strings declaring execution dependencies

## 4. Execution Graph Integration
The Core Orchestrator (orchestration_engine.js) dynamically loads this JSON registry to build its Execution Task Graph (DAG). It iterates over the registry, checks untime-policy.json for eligibility, and executes the validator scripts defined in the registry.
