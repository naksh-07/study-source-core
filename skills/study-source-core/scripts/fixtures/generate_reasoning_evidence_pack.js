 ...
 * from a raw Reasoning source fixture.
 */
...
function generateReasoningEvidencePack(fixtureOrPath, outputDir = null) {
    let fixture = fixtureOrPath;
    let sourceContentForHash = '';

    if (typeof fixture === 'string') {
        if (fs.existsSync(fixture)) {
            sourceContentForHash = fs.readFileSync(fixture, 'utf8');
            fixture = JSON.parse(sourceContentForHash);
        } else {
            sourceContentForHash = fixture;
            fixture = JSON.parse(fixture);
        }
    } else {
        sourceContentForHash = JSON.stringify(fixture, null, 2);
    }
    ...
    lines.push('- Domain: ' + (fixture.domain || 'Reasoning'));
    lines.push('- Topic: ' + (fixture.topic || 'Logical Deduction'));
    lines.push('- Skill ID: ' + (fixture.skill_id || 'reasoning-study'));
    ...
    for (const c of (fixture.concepts || [])) {
        ...
        if (c.conditions) lines.push('- Conditions: ' + c.conditions);
        lines.push('');
    }

    lines.push('## 3. Master Principles & Governing Rules');
    for (const r of (fixture.master_rules || [])) {
        lines.push('### ' + r.name);
    ...
    const sourceHash = computeSha256(sourceContentForHash);

    const provenanceMetadata = {
        detected_subject: fixture.subject,
        chapter_title: fixture.chapter,
        source_hash_sha256: sourceHash,
        evidence_hash_sha256: evidenceHash,
        source_title: fixture.source_provenance ? fixture.source_provenance.source_title : 'Reasoning Source Evidence',
