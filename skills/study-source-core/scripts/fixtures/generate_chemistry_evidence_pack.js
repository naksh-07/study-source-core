 ...
 * from a raw Chemistry source fixture.
 */
...
function generateChemistryEvidencePack(fixtureOrPath, outputDir = null) {
    ...
    lines.push('- Domain: ' + (fixture.domain || 'Chemistry'));
    lines.push('- Topic: ' + (fixture.topic || 'Physical Chemistry'));
    lines.push('- Skill ID: ' + (fixture.skill_id || 'chemistry.physical.equilibrium'));
    ...
    for (const c of (fixture.concepts || [])) {
        ...
        if (c.si_unit) lines.push('- SI Unit: ' + c.si_unit);
        if (c.dimensional_formula) lines.push('- Dimensional Formula: ' + c.dimensional_formula);
        lines.push('');
    }
    ...
    for (const f of (fixture.master_formulas || [])) {
        ...
        if (f.units) lines.push('- Units: ' + f.units);
        lines.push('');
    }
    ...
    const provenanceMetadata = {
        source_name: fixture.source_provenance ? fixture.source_provenance.source_title : 'Chemistry Source Fixture',
        ...
        detected_subject: fixture.subject || 'Chemistry',
        chapter_title: fixture.chapter || 'Chemical-Equilibrium'
    };
