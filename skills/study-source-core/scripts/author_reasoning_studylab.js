...
function parseMarkdownEvidencePackText(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        } else if (line.startsWith('## 3. Master Principles') || line.startsWith('## 3. Master Formulas') || line.startsWith('## 3. Extracted Concepts')) {
...
async function executeReasoningSpecialistTask(task, context = {}) {
    ...
    if (!evidenceInput) {
        evidenceInput = context.evidencePack || null;
    }
